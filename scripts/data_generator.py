"""
This script is used to extract data windows from the raw data using window_extractor.py.
The data windows are saved to a new directory called data_windows.
"""
...
def main():
    ...
    for csv_file in csv_files:
        ...
        # Step data not included
        if "step" in csv_file.name.lower():
            print(f"Skipping (contains 'step'): {rel_path}")
            continue
        ...
        if result.returncode != 0:
            ...
            print(f"  Processed: {result.stdout.strip()}")
